...
{
    public class ManualViewModelRPU : WindowViewModelBaseEx
    {
        #region 私有变量
        ...
        private void SetedValues()
        {
            try
            {
                if (!(GlobalModel.DicDeviceInfo.TryGetValue(EDeviceType.RPU.ToString(), out DeviceInfo_ALL deviceInfo)))
                {
                    MessageBox.Show($"设备初始化异常：【{deviceInfo.GetType()}】");
                    return;
                }

                if (!(deviceInfo.DeviceOperate is IRPU operate))
                {
                ...
                SuperDHHLoggerManager.Info(LoggerType.TESTLOG, nameof(ManualViewModelRPU), nameof(SetedValues), InfoString);
                ...
                if (OperateType != SetOutPutState.Null)
                {
                    ...
                    SuperDHHLoggerManager.Info(LoggerType.TESTLOG, nameof(ManualViewModelRPU), nameof(SetedValues), $"设备已{InfoString}");
                }
            }
            catch (Exception ex)
            {
                SuperDHHLoggerManager.Exception(LoggerType.TESTLOG, nameof(ManualViewModelRPU), nameof(SetedValues), ex);
