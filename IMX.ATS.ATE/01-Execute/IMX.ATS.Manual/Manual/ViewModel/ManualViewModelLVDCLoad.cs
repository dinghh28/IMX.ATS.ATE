...
namespace IMX.ATS.Manual
{
    public class ManualViewModelLVDCLoad : WindowViewModelBaseEx
    {
        ...
        private void SetedValues()
        {
            try
            {
                if (!(GlobalModel.DicDeviceInfo.TryGetValue(EDeviceType.LVDCLoad.ToString(), out DeviceInfo_ALL deviceInfo)))
                {
            ...
            {
                SuperDHHLoggerManager.Exception(LoggerType.TESTLOG, nameof(ManualViewModelLVDCLoad), nameof(SetedValues), ex);
